function checkuniquebroker(ciID, action_type, msgID, btnid) {
    ...
    $.ajax({
        method: "POST",
        url: "./Assets/backendfiles/broker_check.php?action_type=" + action_type + "&broker_name=" + civalue,
        data: {
            user_name: civalue
        },
...
function addCheckCall() {
    ...
    var tdate = new Date()
    var for_date = tdate.getMonth()+1 + "-" + tdate.getDay() + "-" + tdate.getFullYear().toString().substr(-2)
    ...
    if(check_notes == "" || typeof(check_notes) == "undefined"){
        ...
        $.ajax({
        method: "POST",
        url: "./Assets/backendfiles/broker_check.php?action_type=addCheckCall&id=" + userid,
        data: {
        ...
        success: function(data) {
            ...
            if (data.success == 1) {
                ...
                setTimeout(function(){
                    ...
                    $("#check_call_submit").removeAttr('disabled', 'disabled');
                    const inputs = document.querySelectorAll('#checkpoints, #check_notes');
                    var td = '<tr>'
                        + '<td title="Date and time the check call was made to driver.">' + for_date + '<p class="light">' + ttime + '</p></td>'
                        + ' <td title="The user who made the call">'+ data.user +'</td>'
                        + ' <td title="Check points added by the user">' + checkpoints + '</td>'
                        + ' <td title="Check notes added by the user">'+ check_notes + '</td>'
                    ...
                    btn.style.backgroundColor = "var(--button)"
                    $("#alert_msg").delay(30000).hide(500);
                    document.getElementById("check_notes").style = "border: 1px solid #dadada;"
...
function addnewload() {
    ...
    if(check_notes == "" || typeof(check_notes) == "undefined"){
        ...
        $.ajax({
        method: "POST",
        url: "./Assets/backendfiles/broker_check.php?action_type=addnewload&" + userid,
        data: {
            checkpoints: checkpoints,
            check_notes: check_notes
        },
...
function loadlocationStatus(status, pickup_loc, destination, userid) {
    ...
    $.ajax({
        method: "POST",
        url: "./Assets/backendfiles/index.php?action_type=loadtracking&id=" + userid,
        data: {
        ...
        success: function(data) {
            ...
            if (data.success == 1) {
                ...
                // setTimeout(function(){
                    ...
                    $('select[name="update_load_status[]"]').removeAttr('disabled', 'disabled');
...
function createStatusbar(obj) {
    ...
    this.delete = $("<div class='abort delete' data-file_rowID='' > Delete </div>").appendTo(this.statusbar).hide();
...
// check Broker
document.getElementById("brokerName").addEventListener("keyup", function() {
    checkuniquebroker('brokerName', 'checkbrokername', 'brokerNameerror', 'brokersubmit')
})
...
$("body").on('change', ".update_load_location_staus", function(e){
    ...
    var activestatus = $(this).parent()[0].nextElementSibling.children[0].children[selectedIndex]
    ...
    if(selectedIndex == 4){
        for(i=0; i<selectedIndex + 1; i++){
            $(this).parent()[0].nextElementSibling.children[0].children[i].classList.add("issue")
        }
    }else{
        activestatus.classList.add('active_step')
        for(i=0; i<selectedIndex; i++){
            $(this).parent()[0].nextElementSibling.children[0].children[i].classList.add("step_performed")
        }
    }
...
$("body").on('click', "#Update_current_loc", function(e){
    ...
    if(current_loc == "" || typeof(current_loc) == "undefined"){
        ...
        var data = JSON.stringify({current_loc: current_loc, lat: lat, lng:lng, destination: destination, pickup:pickup, distance: distance, originaDistance: originaDistance, duration:duration, truck_no:truck_no})
        
        var userid = $("#load_id").val()
        $.ajax({
            method: "POST",
            url: "./Assets/backendfiles/index.php?action_type=currentlocUpdate&id=" + userid,
            data: {
                data: data
            },
            success: function(data) {
...
$(document).on('submit', "form.new_load_form", function(e) {
    e.preventDefault();    
    var formData = new FormData(this);
    // console.log(formData)
    // window.location.pathname

    $("body .newloadbtn").html("Adding...")
    $("body .newloadbtn").style = "background-color: #dadada;"
    $("body .newloadbtn").attr('disabled', 'disabled');

    $.ajax({
        url: './Assets/backendfiles/index.php?action_type=addnewload',
        type: 'POST',
        data: formData,
        success: function (data) {
            data = JSON.parse(data)[0]
            if(data.success == 1){
                $("#alert_msg").css({display: "block", backgroundColor: "#7dc77d"})
                $("#alert_msg").html(data.msg);
                $("#new_load_form")[0].reset();
                $("#alert_msg").delay(30000).hide(500);

                fetchloadrows(["en_route"], ['table1'])
                // $('#table1').DataTable().destroy();
                // $("#loadBoard1").html(data.rows);
                // $('#table1').DataTable().draw();

                $("body .newloadbtn").html("Submit")
                $("body .newloadbtn").style = "background-color: var(--button);"
                $("body .newloadbtn").removeAttr('disabled', 'disabled');
...
$('body').on("change", "select#broker", function(e){
    ...
    var brokeragent = $(this).parent().parent().parent()[0].children[1].children[1];
...
// Delete Load
$("#loadBoard1, #loadBoard2, #loadBoard3, #loadBoard4, #loadBoard5, #loadBoard6").on('click', '.load_action', function(e){
    ...
    if(confirmed){
        ...
        $.ajax({
        type: "POST",
        url: "./Assets/backendfiles/index.php?action_type="+ action_type + "&id=" + loadID,
        data: loadID,
        success: function(data) {
            data = JSON.parse(data)[0]
            if(data.success == "1"){
                setTimeout(function(){
                    ...
                    fetchloadrows(["en_route", 'delivered', 'issue', 'invoiced', 'paid', 'load_Factored'], ['table1', 'table2', 'table3', 'table4', 'table5', 'table6'])
...
// Get Edit load Form
$("#loadBoard1, #loadBoard2, #loadBoard3, #loadBoard4, #loadBoard5, #loadBoard6").on('click', '.get_load_form', function(e){
    var loadID = $(this).data("load_id");
    var action_type = $(this).data("action_type");
...
$(document).ready(function () {
    ...
    var fieldHTML = '<div class="inputgroup">'
        + '<div class="inputbox">'
    ...
    $("body").on(' change', '.end', function(e) {
        ...
        // console.log(this)
        // console.log($(this).parent().parent().parent()[0].children[0].children[2])
        // console.log($(this).parent().parent().parent()[0].children[0].querySelector(".start"))
        // console.log($(this).parent().parent().parent()[0].children[0].children[2])
        // console.log($(this).parent().parent().parent()[0].children[0].querySelector(".start").value)
        // console.log($('input.end').prevAll('input.start:first').val("woo"))
        // console.log(e.target.nextElementSibling.nextElementSibling)

        var PU_ele = $(this).parent().parent().parent()[0].children[0].querySelector(".start")
        var msg_ele = $(this).parent().parent().parent()[0].children[0].querySelector(".pu_blank_msg")
        var PU_location = $(this).parent().parent().parent()[0].children[0].querySelector(".start").value

        if(PU_location.trim() == null || typeof(PU_location) == "undefined" || PU_location.trim() == ""){
            this.style = "cursor: no-drop; width: 100%;background-color: #dadada;"
            PU_ele.style = "border: 1px solid red;"
            msg_ele.style = "color: red;"
            msg_ele.textContent = "Please add the Pickup location first!"
        } else{
            this.style = "cursor: auto; width: 100%;background-color: transparent;"
            PU_ele.style = "border: 1px solid #dadada;"
            msg_ele.textContent = ""
        ...
        if(typeof(e.which) == "undefined"){
            var directionsService = new google.maps.DirectionsService();
            var directionsRenderer = new google.maps.DirectionsRenderer();
            var dis = e.target.nextElementSibling.nextElementSibling.nextElementSibling
            var dur = e.target.nextElementSibling.nextElementSibling.nextElementSibling.nextElementSibling
            ...
            shortest = [];
            fastest = [];
            console.log("End route directions")
            console.log(PU_location.trim())
    ...
    $('#clear_call_check_form').on('click', function(e){
      $("#checkpoints").val("")
      $("#check_notes").val("")
    })

    $(".tracker_header").on('click', function(e){
    ...
    obj.on('drop', function(e) {
        ...
        handleFileUpload(files, $("#rate_con_files_con"));
    });
    ...
    $(document).on('dragover', function(e) {
        e.stopPropagation();
        e.preventDefault();
        // obj.css({'border': '1px dashed #dadada', 'backgroundColor': '#dadada'});
    });
